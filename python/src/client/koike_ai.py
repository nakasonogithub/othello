...
class Client(WebSocketClient):
    ...
    def received_message(self, msg):
        ...
        message受信時に呼ばれる関数
        """
        print("received_message: {0}".format(msg))
        ...
        if action   == 'role':
            ...
            #
            # 勝敗が送信されてくる
            print("Match result is {0}".format(msg['result']))
            self.close


def where_should_i_place():
    where_can_i_place()
    print("can place list\n" + format(can_place_list))
    print("max strength:{0}".format(max_strength))
...
print("connect to {0}".format(SERV))
